...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	//////////////////////////////////////////////////////////////////////////////////
	int i, j; // i и j количество вертексов и индексов дл€ квадрата из 2 треугольников
	float yOffset(0);
	...
	// сделать от 0 до 5, 6 поинтов, по ним пон€тно в какой позиции должна быть вершина дл€ квадрата из 2 треугольников,тип делишь 6 на Iтое и понимаешь, 
		...
		)
	{
		...
		countSquare++; //счетчик квадратов, так-же использую дл€ X координат выше
		//counts++;

		if (countSquare % columns == 0) //%columns сколько квадратов в строке
		{
			yOffset -= 4; //смещение по координатам вниз дл€ новых строк
			countSquare = 0;
		}
	}
...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	const int stacks = 10;    //  оличество горизонтальных сегментов
	const int slices = 10;    //  оличество вертикальных сегментов

	// ¬ычисл€ем количество вершин и индексов
	m_vertexCount = (stacks + 1) * (slices + 1);
	m_indexCount = stacks * slices * 6; // 2 треугольника на каждый квадрат

	// —оздаем массивы вершин и индексов
	vertices = new VertexType[m_vertexCount];
	if (!vertices)
	{
		return false;
	...
	for (int stack = 0; stack <= stacks; stack++)
	{
		...
		for (int slice = 0; slice <= slices; slice++)
		{
			...
			// ÷вет в зависимости от положени€ на сфере
